...
static const int NUM_LEDS_PER_UNIVERSE = 56; 
...
extern "C" void app_main(void)
{
    ...
    while (1)
    {
        ...
        if (i2s_channel_read(rx_chan, r_buf, BUFFER_SIZE * 2, &r_bytes, 1000) == ESP_OK)
        {
            ...
            // int i = 0;
            // int16_t values[8] = {r_buf[i], r_buf[i+1], r_buf[i+2], r_buf[i+3], r_buf[i+4], r_buf[i+5], r_buf[i+6], r_buf[i+7]};
            // double rms = normalized_rms(values);
            // printf("%d\n", (int)rms);
        }
        else
        {
